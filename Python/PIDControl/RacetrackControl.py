...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        cte = 0
        if(self.x < radius):
            # centor of circle is at (radius , radius)
            # if robot sit on the circle, the distance (x,y) to (radius,radius) is radius
            cte = sqrt((self.x - radius) ** 2 + (self.y - radius)**2) - radius
        elif(self.x > 3 * radius):
            # centor of circle is at (3 * radius , radius)
            # if robot sit on the circle, the distance (x,y) to (3 * radius, radius) is radius
            cte = sqrt((self.x - 3.0 * radius) ** 2 +
                       (self.y - radius)**2) - radius
        elif(self.x >= radius and cos(self.orientation) > 0):
            cte = self.y - 2 * radius
        elif(self.x >= radius and cos(self.orientation) < 0):
            cte = 0 - self.y
        return cte

...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # update Gaussian
            error *= exp(
                -(error_bearing**2) / (self.bearing_noise**2) / 2.0
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        distance_with_noise = random.gauss(motion[1], self.distance_noise)
        steering_with_noise = random.gauss(motion[0], self.steering_noise)
        # print("steering_with_noise",steering_with_noise,motion[0])
        # print("distance_with_noise",distance_with_noise,motion[1])
        if fabs(steering_with_noise) < 1e-10:
            res.x = self.x + distance_with_noise * cos(self.orientation)
            res.y = self.y + distance_with_noise * sin(self.orientation)
            res.orientation = (self.orientation + turning_angle) % (pi * 2)
        else:
            turning_angle = distance_with_noise / self.length * tan(steering_with_noise)
            turning_radius = distance_with_noise / turning_angle
            cx = self.x - sin(self.orientation) * turning_radius
            cy = self.y + cos(self.orientation) * turning_radius
            res.x = cx + sin(self.orientation + turning_angle) * turning_radius
            res.y = cy - cos(self.orientation + turning_angle) * turning_radius
            res.orientation = (self.orientation + turning_angle) % (pi * 2)
    ...
    # according to
    #           self.bearing_noise
    def sense(self, enable_noise=True):  # do not change the name of this function
        Z = []
        ...
        for i in range(len(landmarks)):
            ...
            global_bearing = atan2(delta_y, delta_x)
            if enable_noise:
                noise = random.gauss(0, self.bearing_noise)
            self_bearing = (global_bearing - self.orientation + noise) % (pi * 2.0)
            Z.append(self_bearing)

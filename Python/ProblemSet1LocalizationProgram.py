...
def move(p, motions, prob_move):
    new_p = [[0.0 for col in range(len(p[0]))] for row in range(len(p))]
    # print(new_p)
...
def sense(p, colors, measurement, sensor_right):
    ...
    for y in range(len(p)):
        for x in range(len(p[0])):
            if measurement == colors[y][x]:
                q[y][x] *= sensor_right
            else:
                q[y][x] *= 1 - sensor_right
            prob_sum += q[y][x]
    for y in range(len(p)):
        for x in range(len(p[0])):
            q[y][x] *= 1 / prob_sum
